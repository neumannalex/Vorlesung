...
namespace Tests
{
    ...
    {
        [Fact]
        public void AverageThrowsOnNull()
        {
            Assert.Throws<ArgumentNullException>(() => ProcessMath.Average(null));
        }

        [Fact]
        public void AverageThrowsOnEmpty()
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => ProcessMath.Average(new List<double>()));
        }
        ...
        public void AverageIsCorrect(MeasurementData data)
        {
            var avg = ProcessMath.Average(data.Measurements);
            Assert.Equal(data.ExpectedAverage, avg, 9);
        }
